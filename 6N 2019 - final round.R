...
sixnations <- data.frame(team = c("IRE","ENG","SCO","WAL","FRA","ITA"),rpass = c(84,85,66,83,70,44),stringsAsFactors = FALSE)
...
list_of_matches <- list(c("ITA","FRA"),
                        c("WAL","IRE"),
                        c("ENG","SCO"))
...
#simulate each of the 3 matches, i
for(i in 1:3){
...
if(a_num_tries >= 4) {a_trybp_flag <- 1} else {a_trybp_flag <- 0}
if(b_num_tries >= 4) {b_trybp_flag <- 1} else {b_trybp_flag <- 0}

#is Wales team a, and if so did they win, if so, give them an extra 3 points for the Grand Slam
if(a_teamname == "WAL" & a_win_flag == 1) {a_grandslam_flag <- 1} else {a_grandslam_flag <- 0}

##calculate the league points awarded given the match result##
a_pt <- a_win_flag * 4 + a_losingbp_flag * 1 + a_trybp_flag * 1 + a_grandslam_flag * 3
...
#ranking
if(i == 3){
#########add the present points to those given in the simulated final round of matches
results_df[4,2] <- as.numeric(results_df[4,2]) + 16  #WAL
results_df[2,2] <- as.numeric(results_df[2,2]) + 15  #ENG
results_df[1,2] <- as.numeric(results_df[1,2]) + 14  #IRE
results_df[3,2] <- as.numeric(results_df[3,2]) + 6   #SCO
results_df[5,2] <- as.numeric(results_df[5,2]) + 6   #FRA
results_df[6,2] <- as.numeric(results_df[6,2]) + 0   #ITA
...
png(filename="6N_2019_pts_finalroundsim_boxplot_1.png",width=600,height=600)
boxplot(list(as.numeric(results_working[,4]),
          as.numeric(results_working[,2]),
          as.numeric(results_working[,1]),
          as.numeric(results_working[,5]),
          as.numeric(results_working[,3]),
          as.numeric(results_working[,6])),
        names = c("WAL","ENG","IRE","FRA","SCO","ITA"),
        main = "Distribution of league points",
        sub = "Six Nations 2019 simulation results, n = 10,000: actual results up to round 4 + simulation of final round",
        ylab = "final league points",
        ylim = c(0,30))
dev.off()

#boxplot of standings
png(filename="6N_2019_std__finalroundsim_boxplot_1.png",width=600,height=600)
boxplot(list(as.numeric(results_working[,16]),
             as.numeric(results_working[,14]),
             as.numeric(results_working[,13]),
...
png(filename="6N_2019_VIC_finalroundsim_barplot_1.png",width=600,height=600)
victory_occurrences <- c( sum(results_working[,16] == " 1")/100,
                           sum(results_working[,14] == " 1")/100,
                           sum(results_working[,13] == " 1")/100,
